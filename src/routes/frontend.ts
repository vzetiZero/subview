...
frontendRouter.get("/node-runtime-config.js", (_req, res) => {
  const payload = JSON.stringify(getCurrencyContext());
  res.type("application/javascript").send(`(function(){var config=${payload};window.NODE_RUNTIME_CONFIG=config;window.currency=config.currency_code;window.currency_code=config.currency_code;window.currency_symbol=config.currency_symbol;window.currency_position=config.currency_code==='THB'?'left':'right_space';window.locale=config.locale;document.documentElement.setAttribute('lang',config.language||'th');try{localStorage.setItem('offline-language','th');}catch(e){}function digits(){return config.currency_code==='THB'?2:0;}function rate(){return Number(config.vnd_to_thb_rate||1);}function convert(v){var n=Number(v||0);if(!isFinite(n))return 0;if(config.base_currency_code===config.currency_code)return n;if(config.base_currency_code==='VND'&&config.currency_code==='THB')return n*rate();if(config.base_currency_code==='THB'&&config.currency_code==='VND')return n/rate();return n;}function fmt(v){return new Intl.NumberFormat(config.locale,{style:'currency',currency:config.currency_code,minimumFractionDigits:digits(),maximumFractionDigits:digits()}).format(v);}function fmtNumber(v){return new Intl.NumberFormat(config.locale,{minimumFractionDigits:digits(),maximumFractionDigits:digits()}).format(v);}function trimNumeric(v){return digits()===0?String(Math.round(v)):String(Number(v.toFixed(digits())));}function replaceText(node){if(!node||!node.nodeValue)return;var text=node.nodeValue;if(!text.trim())return;text=text.replace(/([0-9][0-9,\.]*)\s*đ/g,function(_,num){var n=Number(String(num).replace(/,/g,''));return fmt(convert(n));});text=text.replace(/([0-9][0-9,\.]*)\s*VND/gi,function(_,num){var n=Number(String(num).replace(/,/g,''));return fmt(convert(n));});text=text.replace(/Tổng cộng:/g,'ยอดรวม:');text=text.replace(/Tổng nạp từ/g,'ยอดเติมสะสมตั้งแต่');text=text.replace(/Giá:\s*/g,'ราคา: ');text=text.replace(/Price:\s*/g,'ราคา: ');node.nodeValue=text;}function walk(root){var tree=document.createTreeWalker(root||document.body,NodeFilter.SHOW_TEXT,{acceptNode:function(node){if(!node.parentNode)return NodeFilter.FILTER_REJECT;var tag=node.parentNode.nodeName;if(tag==='SCRIPT'||tag==='STYLE'||tag==='NOSCRIPT'||tag==='TEXTAREA')return NodeFilter.FILTER_REJECT;return NodeFilter.FILTER_ACCEPT;}});var current;while(current=tree.nextNode()){replaceText(current);}}function patchDataPrice(){document.querySelectorAll('[data-price]').forEach(function(el){var raw=Number(el.getAttribute('data-price')||0);if(!el.hasAttribute('data-price-vnd')){el.setAttribute('data-price-vnd',String(raw));}var displayValue=convert(Number(el.getAttribute('data-price-vnd')||raw));el.setAttribute('data-price',trimNumeric(displayValue));});}function patchLocaleMeta(){document.querySelectorAll('meta[property="og:locale"]').forEach(function(meta){meta.setAttribute('content','th_TH');});}function forceThaiUi(){var modalLabel=document.getElementById('modalLanguageLabel');if(modalLabel)modalLabel.textContent='เลือกภาษา';document.querySelectorAll('#btn-language span,.offline-i18n-current,.offline-i18n-hint').forEach(function(node){if(node.textContent&&/language|ngôn ngữ/i.test(node.textContent)){node.textContent='เลือกภาษา';}});}function run(){patchLocaleMeta();patchDataPrice();walk(document.body);forceThaiUi();}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',run);}else{run();}})();`);
});
